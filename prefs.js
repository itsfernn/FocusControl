...
export default class Preferences extends ExtensionPreferences {
    ...
    _buildKeybindingsPage(settings) {
        ...
        page.add(this._buildKeybindingsGroup(settings));
        page.add(this._buildBorderStyleGroup(settings));

        return page;
    }

    _buildKeybindingsGroup(settings) {
        const group = new Adw.PreferencesGroup({
            title: "Window Focus Shortcuts",
            description: "Configure directional focus movement shortcuts.",
        });

        group.add(this._shortcutRow(settings, "focus-up"));
        group.add(this._shortcutRow(settings, "focus-left"));
        group.add(this._shortcutRow(settings, "focus-right"));
        group.add(this._shortcutRow(settings, "focus-down"));
    ...
    _buildBorderStyleGroup(settings) {
        const group = new Adw.PreferencesGroup({
    ...
    _spinRow(settings, schemaKey, lower, upper, step) {
        const settingsSchemaKey = settings.settings_schema.get_key(schemaKey);
        ...
        settings.bind(schemaKey, row, "value", Gio.SettingsBindFlags.DEFAULT);

        return row;
    }

    _colorRow(settings, schemaKey) {
        const summary = settings.settings_schema.get_key(schemaKey).get_summary();
        ...
        button.connect("notify::rgba", () => {
            ...
            settings.set_string(schemaKey, newRgba.to_string());
        });
    ...
    _entryRow(settings, schemaKey) {
        ...
        settings.bind(schemaKey, row, "text", Gio.SettingsBindFlags.DEFAULT);
                ...
                switch (response) {
                    case "save":
                        if (acceleratorName === null) {
                            ...
                            settings.set_strv(key, [acceleratorName]);
                        }
                        break;
                    case "cancel":
                        break;
